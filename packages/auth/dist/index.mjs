...
var UserContext = createContext2(void 0);
...
import { useState as useState2, useEffect, useCallback as useCallback2, useMemo as useMemo2, useRef } from "react";
...
function AuthProvider({
  ...
  const supabase = useMemo2(() => createClient({ instance }), [instance]);
  const refreshTimeoutRef = useRef(void 0);
  useEffect(() => {
    setupDebugHelpers();
  }, []);
  ...
  const emitAuthEvent = useCallback2((type, data) => {
    const event = {
      type,
  ...
  const _fetchUserProfile = useCallback2(
    async (userId) => {
      try {
  ...
  const handleSessionChange = useCallback2(
    async (session) => {
  ...
  const signInWithOtp = useCallback2(
    async (email, options) => {
      return requestOtp({
        identifier: email,
...
import { useState as useState3, useEffect as useEffect2, useCallback as useCallback3, useMemo as useMemo3, useRef as useRef2 } from "react";
...
function UserProvider({
  ...
  const [brands, setBrands] = useState3(
    null
  );
  const [loading, setLoading] = useState3(false);
  const [error, setError] = useState3(null);
  const [lastFetch, setLastFetch] = useState3(0);
  const realtimeChannelRef = useRef2(null);
  const fetchUserBrands = useCallback3(
  ...
  const fetchUserData = useCallback3(
    async (force = false) => {
      console.log(
  ...
  const verificationState = useMemo3(() => {
  ...
  const isFullyVerified = useMemo3(() => {
    return verificationState?.isFullyVerified ?? false;
  }, [verificationState]);
  const updatePendingEmail = useCallback3(
    async (email) => {
      if (!profile?.id) {
  ...
  const updatePendingPhone = useCallback3(
    async (phone) => {
      if (!profile?.id) {
        throw new UserProfileError("No user profile available");
      }
  ...
  const cancelPendingChange = useCallback3(
    async (type) => {
      if (!profile?.id) {
        throw new UserProfileError("No user profile available");
      }
  ...
  const markContactVerified = useCallback3(
    async (type) => {
      if (!profile?.id) {
        throw new UserProfileError("No user profile available");
      }
  ...
  const updateSelectedBrand = useCallback3(
    async (brandId) => {
      if (!profile?.id) {
        throw new UserProfileError("No user profile available");
      }
  ...
  const hasAppAccess2 = useCallback3(
    (app) => {
  ...
  return /* @__PURE__ */ jsx2(UserContext.Provider, { value: contextValue, children });
}
